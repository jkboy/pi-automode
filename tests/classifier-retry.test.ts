...
/** fakeComplete that never resolves until the request signal aborts. */
function fakeCompleteHangingThen(responses: Array<AssistantMessage | Error>) {
	let calls = 0;
	const fn = async (
		_model: unknown,
		_prompt: unknown,
		...
		if (item instanceof Error && item.message === "__hang__") {
...
const STAGE_PROMPT = {
	...
	actionMessage: buildClassifierActionMessage("exact action"),
};

const CLASSIFIER = { model: { provider: "test", id: "x" } } as never;
...
test("fast stage retries transient thrown errors with doubling backoff and recovers", async () => {
	...
	const decision = await classifyInStages(fn, CLASSIFIER, STAGE_PROMPT, undefined, {
		sessionId: "s",
...
test("fast stage fails closed immediately on a non-retryable auth error", async () => {
	...
	assert.match(decision.reason, /Fast classifier failed.*401/);
...
test("maxAttempts of 1 disables transient retries", async () => {
	const { fn, callCount } = fakeCompleteOrThrow([new Error("socket hang up"), assistantWith("0")]);
	const decision = await classifyInStages(fn, CLASSIFIER, STAGE_PROMPT, undefined, {
		sessionId: "s",
...
test("classifier timeout is a transient error: the next attempt gets a fresh timeout budget", async () => {
	const { fn, callCount } = fakeCompleteHangingThen([
		new Error("__hang__"),
		assistantWith("0"),
	]);
	const attempts: ClassifierIoAttempt[] = [];
	...
	assert.equal(decision.decision, "allow");
	assert.equal(callCount(), 2);
	assert.match(attempts[0]?.error ?? "", /timed out after 30 ms/);
	assert.equal(attempts[0]?.retryDelayMs, 0);
});
...
test("fast stage never trusts a digit from a truncated response", async () => {
	const { fn, callCount } = fakeCompleteOrThrow([assistantWith("0", "length")]);
	const decision = await classifyInStages(fn, CLASSIFIER, STAGE_PROMPT, undefined, {
		sessionId: "s",
		retry: { maxAttempts: 3, baseDelayMs: 0 },
	});
...
test("fast stage keeps failing closed on an aborted response without retrying", async () => {
	const { fn, callCount } = fakeCompleteOrThrow([assistantWith("", "aborted")]);
	const decision = await classifyInStages(fn, CLASSIFIER, STAGE_PROMPT, undefined, {
		sessionId: "s",
		retry: { maxAttempts: 3, baseDelayMs: 0 },
	});
...
test("detailed stage fails closed immediately on insufficient_quota", async () => {
	...
	assert.equal(decision.decision, "block");
	assert.match(decision.reason, /^Classifier failed.*insufficient_quota/);
	assert.equal(callCount(), 2);
});

test("detailed stage transient budget is independent of the fast stage's", async () => {
	const { fn, callCount } = fakeCompleteOrThrow([
...
test("invalid classifierRetry values fall back to defaults and surface diagnostics", () => {
	const config = buildEffectiveConfigFromSources({
		globalSettings: [
			...
			{ autoMode: { classifierRetry: [1, 2] } } as never,
		],
	});
	assert.deepEqual(config.classifierRetry, DEFAULT_CLASSIFIER_RETRY);
	...
	assert.equal(
		validateSettingsFile({ autoMode: { classifierRetry: [] } } as never, "t.json")
			.filter((d) => d.includes("classifierRetry must be an object")).length,
		1,
	);
});
...
test("retry helpers: blacklist matching and backoff math", () => {
	...
	assert.equal(isNonRetryableClassifierError("Classifier request timed out after 20000 ms."), false);
